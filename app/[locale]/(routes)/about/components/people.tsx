            ...
            return (
              <article key={person.key} className="flex flex-col">
                <div className="aspect-4/5 overflow-hidden">
                  <Image src={person.image} alt={t(`people.${person.key}.alt`)} width={900} height={1125} className="h-full w-full object-cover" />
                </div>

                <div className="pt-4">
                  <p className="text-sm text-(--darkbrown)">{t(`people.${person.key}.role`)}</p>

                  <p className="pt-2 text-sm uppercase tracking-[0.08em] text-(--darkbrown)">{t(`people.${person.key}.name`)}</p>
                  ...
                  <details className="pt-4 group">
                    ...
                    <div className="pt-4">
                      <p className="text-sm text-(--darkbrown)">{languages.join(", ")}</p>

                      <a href={`mailto:${person.email}`} className="block pt-2 text-sm text-(--darkbrown)">
                        {person.email}
                      </a>

                      <a href={`tel:${person.phone.replace(/\s+/g, "")}`} className="block pt-2 text-sm text-(--darkbrown)">
                        {person.phone}
                      </a>
                    </div>
                  </details>
